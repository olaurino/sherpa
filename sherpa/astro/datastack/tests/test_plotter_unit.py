...
def patch_module(request):
    ...
    """
    clean_modules()
    # Apparently, newer versions of astropy naviagate the system modules and fail is they can't find __file__
    sys.modules['chips_backend'] = MagicMock(spec=plot.dummy_backend, __file__="foo.bar")
    sys.modules['sherpa.plot.pylab_backend'] = MagicMock(spec=plot.dummy_backend, __file__="foo.bar")
    sys.modules['pychips'] = MagicMock(__file__="foo.bar")
    sys.modules['matplotlib'] = MagicMock(__file__="foo.bar")
    sys.modules['matplotlib.pyplot'] = MagicMock(__file__="foo.bar")
...
class PyChipsSpec(object):
    ...
    chips_noline = stub
    chips_solid = stub
    chips_shortdash = stub
    chips_longdash = stub
    chips_dotlongdash = stub
    chips_none = stub
